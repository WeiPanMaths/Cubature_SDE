...
//************************************
void CCloudWeightLocation::DoAbstractPruningMultithreaded
//void DoAbstractPruningMultithreaded
	 ...
	 std::map<size_t, size_t> & miIn2Out)
{
	...
	NewWeights.resize(iNoDimensionsToCubature);
...
void CCloudWeightLocation::DoAbstractPruningUpdate
	(std::vector<std::set<CWeightLocation, CWeightLocationCompare>::iterator>& vplwToBeProcessed,
	std::vector<double>& NewWeights, 
	std::map<size_t, size_t> & miIn2Out,
	...
	)
{
	...
	for (size_t j = 0; j < noActiveWeightsLocations; j++)
	{
...
CCloudWeightLocation& CCloudWeightLocation::ReducePointSetMultithreaded(const double& dAccuracySpread,
														   ...
														   CMainProgramParameters& ipIntParams)
{
	...
	auto noOfPatches = vvplwToBeProcessed.size();
	std::vector< std::vector<double> >		vNewWeights(noOfPatches);
	std::vector< std::map<size_t, size_t> > vmiIn2Out(noOfPatches);
	std::vector< size_t >					vNoActiveWeightsLocations(noOfPatches);

	// runs recombine in multithreaded fashion
	boost::thread_group threads;
	for (size_t uiPatchNo = 0; uiPatchNo < noOfPatches; ++uiPatchNo)
	{
		std::vector<BASE::iterator>&	vplwToBeProcessed(vvplwToBeProcessed[uiPatchNo]);
		std::vector<double>&			NewWeights(vNewWeights[uiPatchNo]);
		std::map<size_t, size_t>&		miIn2Out(vmiIn2Out[uiPatchNo]);
		vNoActiveWeightsLocations[uiPatchNo] = vplwToBeProcessed.size();

		/*auto recomb_threaded = [&vplwToBeProcessed, &NewWeights, &miIn2Out, &iNoDimensionsToCubature, &ipIntParams]() {
	 		DoAbstractPruningMultithreaded(vplwToBeProcessed, iNoDimensionsToCubature, ipIntParams, NewWeights, miIn2Out);
		};*/
		auto recomb_threaded = [&]() {
	 		DoAbstractPruningMultithreaded(vplwToBeProcessed, iNoDimensionsToCubature, ipIntParams, NewWeights, miIn2Out);
		};
		threads.create_thread( recomb_threaded );
	}
	threads.join_all();
	...
	for (size_t uiPatchNo = 0; uiPatchNo < noOfPatches; ++uiPatchNo)
	{
		std::vector<BASE::iterator>&	vplwToBeProcessed(vvplwToBeProcessed[uiPatchNo]);
		std::vector<double>&			NewWeights(vNewWeights[uiPatchNo]);
		size_t&							noActiveWeightsLocations(vNoActiveWeightsLocations[uiPatchNo]);
		std::map<size_t, size_t>&		miIn2Out(vmiIn2Out[uiPatchNo]);

		DoAbstractPruningUpdate(vplwToBeProcessed, NewWeights, miIn2Out, noActiveWeightsLocations );
	}
